...
function getValue(students) {
    students.forEach((studen, index) => {
        const tr = document.createElement('tr');
       
        tr.innerHTML = `
        <td rowspan="${studen.total_off}">${index + 1 }</td>
        <td rowspan="${studen.total_off}">${studen.name}</td>
        <td rowspan="${studen.total_off}">${studen.email}</td>
        <td rowspan="${studen.total_off}">${studen.phone}</td>
        <td rowspan="${studen.total_off}" class="text-center">${studen.total_off}/${studen.sessions}</td>
        <td class="text-center">${studen.detail_info[0].date}</td>
        <td>${studen.detail_info[0].note}</td>
        <td>${studen.detail_info[0].teacher}</td>
        `
        body.appendChild(tr)

        if (studen.detail_info.length > 1) {
...
getValue(student_warning.students)